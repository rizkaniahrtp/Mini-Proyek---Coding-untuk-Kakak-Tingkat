...
char password[30]; //variabel untuk input password dari pengguna
int percobaan = 0; //variabel untuk memyimpan history user yang melakukan percobaan menginput password berapa kali

//fungsi
void menunggu(int detik){ //fungsi menunggu
    time_t start = time(NULL); //waktu dimulai saat fungsi dipanggil (waktu sekarang)
    while(time(NULL) - start < detik); //mengulang untuk kondisi jika waktu yang telah berlalu lebih kecil dari detik yang telah ditentukan 
}

void passwordSalah(){ //fungsi passwordSalah
    while(strcmp(passwordBenar, password) != 0){ //mengulang untuk kondisi jika passwordBenar dan password tidak memiliki isi string yang sama
